...
server_params_help = """
Enter either:  
• HTTP/HTTPS URL for SSE connection (e.g., http://localhost:8080/sse)  
or  
• JSON configuration for stdio connection. Sample format:  
```json  
{
"command": "uvx",  
        ...
        if artifacts or response.get("model-thoughts"):
            with st.expander("See details"):
                ...
                if response.get("model-thoughts"):
                    st.info(f"💭 :orange-badge[**Model thoughts:**]\n\n{response['model-thoughts']}")                    
